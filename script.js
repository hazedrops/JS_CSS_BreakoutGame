...
function moveBall() {
  ...
  // If the ball touches left end or right end of the wall
  ...
  if (ball.y + ball.size > canvas.height || ball.y - ball.size < 0) {
    ball.dy *= -1 // Reverse the horizontal direction
  }
  ...
  bricks.forEach(column => {
    column.forEach(brick => {
      // Make it bounce if brick is visible
      if(brick.visible) {
        if(ball.x - ball.size > brick.x &&  // Left brick side check
          ...
          ball.y - ball.size < brick.y + brick.h  // bottom brick side check 
            ...
            ball.dy *= -1 // Bounce off the brick to the opposite direction
...
function increaseScore() {
  ...
  // If there's no bricks left, then show all the bricks again
...
function keyUp(e) {
  if(e.key === 'Right' || e.key === 'ArrowRight' || e.key === 'Left' || e.key === 'ArrowLeft') {
    paddle.dx = 0 // Stop moving when any key is up
  }
